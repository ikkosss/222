...
export default function AddOperatorScreen() {
  ...
  const handleSave = async () => {
    if (!operatorName.trim()) {
      Alert.alert('Ошибка', 'Введите название оператора');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        Alert.alert('Успех', 'Оператор добавлен', [
          { text: 'OK', onPress: () => router.back() }
        ]);
      } else {
        const errorData = await response.json();
        Alert.alert('Ошибка', errorData.detail || 'Не удалось добавить оператора');
      }
    } catch (error) {
      console.error('Error creating operator:', error);
      Alert.alert('Ошибка', 'Произошла ошибка при добавлении оператора');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      >
        <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
          <View style={styles.header}>
            ...
            <Text style={styles.title}>Добавление нового оператора</Text>
          </View>

          <View style={styles.content}>
            {/* Operator Name Input */}
            <View style={styles.inputGroup}>
              <Text style={styles.label}>Название оператора</Text>
              <TextInput
