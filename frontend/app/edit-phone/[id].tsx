...
export default function EditPhoneScreen() {
  ...
  const loadPhoneData = async () => {
    try {
      const response = await fetch(`${BACKEND_URL}/api/phones/${phoneId}`);
      if (response.ok) {
        const phoneData: Phone = await response.json();
        setPhoneNumber(phoneData.number);
        setSelectedOperator(phoneData.operator_id);
      } else {
      ...
      Alert.alert('Ошибка', 'Не удалось загрузить данные номера');
      router.back();
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        Alert.alert('Успех', 'Номер телефона обновлен', [
          { text: 'OK', onPress: () => router.back() }
        ]);
      } else if (response.status === 404) {
        Alert.alert('Ошибка', 'Номер или оператор не найден');
      } else {
        const errorData = await response.json();
        Alert.alert('Ошибка', errorData.detail || 'Не удалось обновить номер');
      }
    } catch (error) {
      console.error('Error updating phone:', error);
      Alert.alert('Ошибка', 'Произошла ошибка при обновлении номера');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDelete = () => {
    Alert.alert(
      'Удалить номер',
      `Вы уверены, что хотите удалить номер ${phoneNumber}?`,
      [
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (response.ok) {
        Alert.alert('Успех', 'Номер телефона удален', [
          { text: 'OK', onPress: () => router.replace('/phones') }
        ]);
      } else {
      ...
      >
        <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
          <View style={styles.header}>
            ...
            <Text style={styles.title}>Редактирование номера</Text>
